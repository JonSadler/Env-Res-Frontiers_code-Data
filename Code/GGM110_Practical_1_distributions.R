...
# ANNOTATE YOUR ANSWERS HERE [use the first rank order curve]
#
#
#
#
...
# And if we use a base2 log scale on the X axis....?
# replot the figure....
ggplot(BMS_rn_abund, aes(x = Number_rec)) + # this sets up the basic plot
  geom_histogram(binwidth = 1,colour="white", fill="black") + # This codes makes the histogram and specifies the bar width, outline and fill colour  
  scale_x_continuous(trans = "log2")+ # this displays the data on an axis scaled to log base 2
  scale_y_continuous(breaks=c(0,2,4,6,8)) + # this makes sure the Y axis counts of species are displayed as integers
  ylab("Number of Species") + xlab("Abundance/count of individuals") # this adds axis labels
# Hump-shaped but with some variability - little skew indicating over representation of species that 
# are characterised by lower abundances, with a few very highly abundant species [see left of the plot]
