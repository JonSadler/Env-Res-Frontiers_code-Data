...
# We're going to install them all the packages we need in one go.
# We should check beforehand but the code is complex.
...
# Now were ae going to load them into the memory
library(dplyr)
library(ggplot2)
library(tidyr)
library(ggmap)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# I've preprocessed out all of the messy stuff and stored it as a CSV file (same format as the infiltration data you used with Dr Kettridge)
...
BMS  <- read.csv(file.choose(), header=T)
...
# We've alreadhy loaded the dplyr,tidyr and ggplot2 libraries 
...
map3 <- ggplot(BMS_rn_abund,aes(x=Number_rec)) + geom_histogram()+labs(y="Count",x="Number of Records")+theme_bw()
...
hist(BMS_rn_abund$Log_Number_rec, main="Species-Abundance plot", 
     ...
     col="black",
     xlim = c(0,20))
map4 <- ggplot(BMS_rn_abund,aes(x=Log_Number_rec)) + geom_histogram()+labs(y="Count",x="Log Number of Records") + theme_bw()
...
BMS_time  <- read.csv(file.choose(), header=T)
...
# We can infer from this that generalists have bigger populations that are more variable (i.e. bigger spread acros the numbers)
# Specialists have smaller populations but are less variable year to year.

View(Butterfly_test) # look at the new datafile
# Run a Wilcoxon test to see if the trends are different. Note what the test is asking.
# We are asking if the populations of the two different groups within each year differ significantly NOT 
# 'Do the trends differ'. That would require a different test comparing the slopes of a regression line.
 
wilcox.test(Population ~ Group, data = Butterfly_test, paired = TRUE) # We add paired is TRUE because we are comparing the same years for each group
# Your going to get a warning messages. THIS IS NOT AN ERROR. It is telling us that there are some pairs in the ranked ties.
# Nothing to worry about!

# The p-value might look odd too. 
# The suffux (e-08) means take the decimal point and move 8 spaces to the left [adding in zeros]

# [So what is the test telling us?]
